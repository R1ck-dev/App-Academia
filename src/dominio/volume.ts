 ...
 * NÃO entrou na conta.
 *
 * A regra que organiza o arquivo inteiro: **o total sozinho é mentira por
 * omissão**. Metade do treino dele não tem carga — abdominal é peso corporal,
 * esteira é tempo —, e um número seco de grama·rep esconde isso. Por isso
 * nenhuma função daqui devolve um `number` solto de volume de sessão:
 * `VolumeDaSessao` carrega o total MAIS a contagem nomeada do que ficou de
 * fora, e a tela é obrigada a dizer "e mais 3 exercícios sem carga".
 *
 * Puro: recebe dado, devolve dado. Sem React, sem SQLite.
 */

import type { TipoMedicao, TipoSerie } from '../db/schema.ts';
import { formatarVolume, proximaCarga, type Carga } from './carga.ts';
import { cargaCompativel, incrementoPadrao, temCarga, type Exercicio } from './exercicio.ts';
...
export type MotivoForaDoVolume = 'aquecimento' | 'sem_carga' | 'sem_repeticoes';

export type ResultadoVolume =
  | { readonly conta: true; readonly gramasReps: number }
 ...
 * "série sem carga".
 */
export function volumeDaSerie(s: SerieExecutada): ResultadoVolume {
  ...
  return { conta: true, gramasReps: s.carga.gramas * s.repeticoes };
}
...
export type VolumeDaSessao = {
  ...
  /** Nomeado, para a UI não inventar texto nem confundir abdominal com esteira. */
...
export function volumeDaSessao(series: readonly SerieComExercicio[]): VolumeDaSessao {
  ...
  // séries sem carga, e fundir os dois num item só apagaria a distinção que
  // estes contadores existem para preservar.
  const fora = new Map<string, { id: string; nome: string; motivo: MotivoForaDoVolume; series: number }>();

  for (const s of series) {
    const r = volumeDaSerie(s);
...
/**
 * Volume DENTRO de um exercício, em grama·rep. `null` onde não há carga:
 * abdominal e esteira não têm volume, e devolver zero seria dizer que ele não
 * fez esforço nenhum.
 */
export type VolumeNaUnidade = { readonly valor: number };

export function volumeDoExercicio(h: HistoricoDoExercicio): VolumeNaUnidade | null {
  if (!temCarga(h.exercicio.tipoMedicao)) return null;
  ...
  for (const s of h.series) {
    ...
    valor += s.carga.gramas * s.repeticoes;
    contou = true;
  }
  return contou ? { valor } : null;
}

export function formatarVolumeNaUnidade(v: VolumeNaUnidade): string {
  return formatarVolume(v.valor);
}
...
export function progressaoDoExercicio(h: HistoricoDoExercicio): PontoDeProgressao[] {
  ...
  for (const s of h.series) {
    ...
    if (
      ...
      (ponto.melhorCarga === null || s.carga.gramas > ponto.melhorCarga.gramas)
    ) {
 ...
 * desenha. Em exercício com carga é a carga em grama; em peso corporal é a
 * repetição; em tempo e distância, o segundo.
 *
...
export function valorDaProgressao(p: PontoDeProgressao, ex: Exercicio): number {
  if (temCarga(ex.tipoMedicao)) return p.melhorCarga === null ? 0 : p.melhorCarga.gramas;
...
export function sugerirCarga(h: HistoricoDoExercicio, alvo: Alvo): Carga | null {
  ...
  return proximaCarga(ultima.carga, incremento);
}
